...
{
	auto [grammar, nonTerminals, terminals] = GetRules(input);
	ValidateRules(grammar, nonTerminals);
	ValidateGrammar(grammar);

	std::tie(grammar, nonTerminals, terminals) = MakeReplenished(grammar, nonTerminals, terminals);
	std::tie(grammar, terminals) = RemoveEmptyRules(grammar, terminals);
	return grammar;
}
